        ...
        if (preg_match_all('/\$\{(.*?)\}/', $templateBody, $vars)) {
            ...
            foreach ($vars[1] as $pos => $var) {
                ...
                switch ($base) {
                    case 'doklad':
                        if ($prop) {
                            $templateBody = str_replace($key, $this->document->getDataValue($prop), $templateBody);
                        }
                        break;
                    ...
                    case 'titulJmenoPrijmeni':
                        $user = new \AbraFlexi\RO(\AbraFlexi\RO::code($this->document->user), ['evidence' => 'uzivatel', 'autoload' => true]);
                        if ($prop) {
                            $templateBody = str_replace($key, $user->getDataValue($prop), $templateBody);
                        } elseif ($base == 'uzivatelJmeno') {
                            $templateBody = str_replace($key, $user->getDataValue('jmeno'), $templateBody);
                        } elseif ($base == 'uzivatelPrijmeni') {
                            $templateBody = str_replace($key, $user->getDataValue('prijmeni'), $templateBody);
                        } elseif ($base == 'titulJmenoPrijmeni') {
                            $templateBody = str_replace($key, trim($user->getDataValue('titul') . ' ' . $user->getDataValue('jmeno') . ' ' . $user->getDataValue('prijmeni') . ' ' . $user->getDataValue('titulZa')), $templateBody);
                        }
                        break;
                    case 'application':
                        $templateBody = str_replace($key, \Ease\Functions::cfg('APP_NAME'), $templateBody);
                        break;
                    case 'nazevFirmy':
                    case 'company':
                        $company = new \AbraFlexi\Company($this->document->company);
                        if ($prop) {
                            $templateBody = str_replace($key, $company->getDataValue($prop), $templateBody);
                        } else {
                            $templateBody = str_replace($key, $company->getDataValue('nazev'), $templateBody);
                        }
                        break;
                    default:
                        $this->addStatusMessage(sprintf(_('Unknown template %s variable: %s'), $this->template, $key), 'debug');
                        break;
                }
            }
        }
